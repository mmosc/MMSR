...
def get_cosine_similarity(arr_a: np.array, arr_b: np.array):
    ...
    norms_b = np.linalg.norm(
        arr_b, axis=-1
    )  # todo why doesn't norms[indicies_test] work here?
...
def get_jaccard_similarity(arr_a: np.array, arr_b: np.array):
    def func(d1, d2, divisor):
        ...
        return np.divide(
            d1_d2_product,
            divisor - d1_d2_product,
            np.zeros_like(divisor),
            where=divisor > 0,
        )
...
def distanceToSongsImproved(idSong, similarity_function, df, features_vector):
    if idSong in df.columns.values:
        ...
        distances = similarity_function(
            features_vector.to_numpy(),
            features_vector.loc[idSong].values.reshape(-1, 1).T,
        ).T[0]
        df[idSong] = distances
...
def meanAveragePrecision(dfTopIds, topNumber):
    ...
    for idx, queryId in enumerate(dfTopIds.index.values):
        ...
        querySong = (
            genres.loc[[queryId]].join(info, on="id", how="left").head(topNumber)
        )
        topSongs = genres.loc[topIds].join(info, on="id", how="left").head(topNumber)
        relevant_results = [
            isResultRelevant(querySong["genre"].values[0], genres)
            for genres in topSongs["genre"].values
        ]
        ...
        if REL == 0:  # Case when there is no relevant result in the top@K
            ...
            AP = (1 / REL) * np.sum(
                [
                    relevant_results[i] * (np.sum(relevant_results[: i + 1]) / (i + 1))
...
dataVectorsMapping = {"tfidf": tf_idf, "bert": bert, "word2vec": word2vec}
...
if exists(csv_topIdsFiles["innerProduct_tfidf"]):
    top_innerProduct_tfidf = pd.read_csv(
        csv_topIdsFiles["innerProduct_tfidf"], index_col=0
    )
else:
...
if exists(csv_topIdsFiles["cosineSim_word2vec"]):
    top_cosine_word2vec = pd.read_csv(
        csv_topIdsFiles["cosineSim_word2vec"], index_col=0
    )
else:
    top_cosine_word2vec = pd.DataFrame(columns=range(100))

if exists(csv_topIdsFiles["innerProduct_word2vec"]):
    top_innerProduct_word2vec = pd.read_csv(
        csv_topIdsFiles["innerProduct_word2vec"], index_col=0
    )
else:
    top_innerProduct_word2vec = pd.DataFrame(columns=range(100))

if exists(csv_topIdsFiles["jaccardSim_word2vec"]):
    top_jaccard_word2vec = pd.read_csv(
        csv_topIdsFiles["jaccardSim_word2vec"], index_col=0
    )
else:
...
if exists(csv_topIdsFiles["innerProduct_bert"]):
    top_innerProduct_bert = pd.read_csv(
        csv_topIdsFiles["innerProduct_bert"], index_col=0
    )
else:
...
@app.get("/query/")
async def getTopResults(
    artist: str,
    track: str,
    top: int,
    vectorData: DataVector,
    simFunction: SimilarityFunction,
):
    print(
        "Get Top results for \n\tArtist: ",
        artist,
    ...
    if id_song in topIdsFiles[file_id].index.values:
        ...
        distanceToSongsImproved(
            id_song, simFunction.to_func(), queryDistances[file_id], vectorData.to_df()
        )
        print("Actual number of records:", len(topIdsFiles[file_id].index))
        # Add new record to top id file
        topIdsFiles[file_id].loc[id_song] = (
            queryDistances[file_id][[id_song]]
            .drop(axis=0, index=[id_song])
            .sort_values(by=id_song, ascending=False)
            .head(100)
            .index.values
        )
    ...
    topVal = (
        genres.loc[topIdsFiles[file_id].loc[id_song].values]
...
@app.get("/metrics/")
async def getEvaluationMetrics(
    vectorData: DataVector, simFunction: SimilarityFunction, k: int
):
    print(
        "Get metrics for vectorData:",
        vectorData,
        "Using similarity funtion",
        simFunction.name,
    )
    ...
    return {
        "SIM": simFunction.to_func(),
        "vectorData": vectorData.to_df(),
        "MAP": pk,
        "MRR": mrrk,
        "NDCG": ndcgk,
    }
